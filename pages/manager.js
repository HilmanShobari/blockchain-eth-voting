...
const ManagerPage = () => {
  ...
  const handleManageVoters = (voting) => {
          ...
          <Card.Content>
            ...
            <Card.Meta style={{ marginTop: '10px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '10px' }}>
                <div><Icon name="list" /> Choices: {voting.totalChoices}</div>
                <div><Icon name="users" /> Voted: {voting.totalVotersVoted}</div>
                <div><Icon name="calendar" /> Start: {voting.fromDate}</div>
                <div><Icon name="calendar check" /> End: {voting.endDate}</div>
              </div>
            </Card.Meta>
            <Card.Description style={{ marginTop: '15px' }}>
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <Button 
                  ...
                  icon 
                  labelPosition="left"
                  onClick={() => handleManageVoters(voting)}
                >
                  <Icon name="user plus" />
                  Manage Voters
                </Button>
        ...
        <Container style={{ padding: '20px 0' }}>
          <div style={{ 
            ...
            <p style={{ color: '#e53e3e' }}>
              {error || 'Unable to connect to the blockchain. Please check your wallet connection.'}
            </p>
          </div>
        </Container>
      </Layout>
      ...
      <Layout>
        <Container style={{ padding: '20px 0' }}>
          ...
          >
            <Modal.Header>
              <Icon name="users" />
              Manage Voters - {selectedVoting?.title}
            </Modal.Header>
            <Modal.Content>
              {selectedVoting && (
                <div>
                  <div style={{ marginBottom: '20px' }}>
                    <AddAllowedVotersForm address={selectedVoting.address} />
                  </div>
                  <div>
                    <CompletedVoteForm address={selectedVoting.address} />
                  </div>
                </div>
              )}
            </Modal.Content>
